...
create or replace table airflow_db_{{ params.env }}.dimensions.{{ params.table }}_purged_smartclip_rows as
(
select
    placementid,
    idcampaign,
...
from
    airflow_db_{{ params.env }}.dimensions.{{ params.table }}
where
    iab_flag = 's'
);
...
update airflow_db_{{ params.env }}.dimensions.{{ params.table }} as c
